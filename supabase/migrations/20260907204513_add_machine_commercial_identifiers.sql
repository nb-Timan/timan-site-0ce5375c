-- Commercial identifiers are independent. SP is a warranty number, MO is a
-- historic machine-order reference, and ERP identifiers are imported sales data.
alter table public.warranty_registrations
  add column if not exists legacy_erp_order_number text,
  add column if not exists legacy_invoice_number text,
  add column if not exists legacy_revenue numeric,
  add column if not exists legacy_contribution_margin_amount numeric,
  add column if not exists legacy_sales_updated_at timestamptz;
...
revoke all on function public.enrich_legacy_machine_sales(jsonb) from public;
...
with legacy_commercial as (
  select distinct on (upper(regexp_replace(wr.machine_serial_number, '[^A-Za-z0-9]+', '', 'g')))
    ...
    wr.legacy_warranty_reference machine_order_number,
    wr.legacy_erp_order_number erp_order_number,
    wr.legacy_invoice_number invoice_number,
    wr.legacy_revenue revenue,
  ...
  select distinct on (upper(regexp_replace(wr.machine_serial_number,'[^A-Za-z0-9]+','','g')))
    ...
    wr.dealer_account_number dealer_number,
    wr.customer_name,
    lc.machine_order_number,
    lc.erp_order_number,
    null::text portal_order_number,
    lc.invoice_number,
    lc.revenue,
    lc.contribution_margin_amount,
    coalesce(wr.is_demo,false) is_demo,
    wr.delivery_date,
    ...
    case when wr.source <> 'legacy_machine_import'
      then case when wr.sharepoint_form_id is not null then 'SP-' || wr.sharepoint_form_id::text else 'SP-' || coalesce(wr.sharepoint_item_id,left(wr.id::text,8)) end
      else null end warranty_id,
  ...
  select * from assessed where
    (nullif(trim(p_query),'') is null or serial ilike '%'||trim(p_query)||'%' or coalesce(warranty_id,'') ilike '%'||trim(p_query)||'%' or coalesce(machine_order_number,'') ilike '%'||trim(p_query)||'%' or coalesce(erp_order_number,'') ilike '%'||trim(p_query)||'%' or coalesce(portal_order_number,'') ilike '%'||trim(p_query)||'%' or machine_model ilike '%'||trim(p_query)||'%')
  ...
  select *,count(*) over()::integer filtered_total,row_number() over(order by
    ...
    case when p_sort='machineOrder' and p_direction='asc' then machine_order_number end asc nulls last,case when p_sort='machineOrder' and p_direction='desc' then machine_order_number end desc nulls last,
    case when p_sort='erpOrder' and p_direction='asc' then erp_order_number end asc nulls last,case when p_sort='erpOrder' and p_direction='desc' then erp_order_number end desc nulls last,
    case when p_sort='portalOrder' and p_direction='asc' then portal_order_number end asc nulls last,case when p_sort='portalOrder' and p_direction='desc' then portal_order_number end desc nulls last,
    ...
    case when p_sort='customer' and p_direction='asc' then customer_name end asc nulls last,case when p_sort='customer' and p_direction='desc' then customer_name end desc nulls last,
