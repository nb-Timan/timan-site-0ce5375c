-- All ordering happens here, before the page slice is taken. This replaces the
-- commercial-fields wrapper so numeric commercial fields can sort correctly.
...
with legacy_commercial as (
  ...
  select distinct on (upper(regexp_replace(wr.machine_serial_number,'[^A-Za-z0-9]+','','g')))
    ...
    wr.dealer_account_number dealer_number, wr.customer_name, lc.machine_order_number, lc.erp_order_number,
  ...
  select *,count(*) over()::integer filtered_total,row_number() over(order by
    ...
    case when p_sort='invoice' and p_direction='asc' then invoice_number end asc nulls last, case when p_sort='invoice' and p_direction='desc' then invoice_number end desc nulls last,
