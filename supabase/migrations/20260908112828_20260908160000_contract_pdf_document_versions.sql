...
begin
  if p_document_kind not in ('draft', 'final') then raise exception 'invalid document kind' using errcode = '22023'; end if;
  if p_language not in ('da', 'en', 'de') then raise exception 'unsupported contract language' using errcode = '22023'; end if;
  if not public.can_generate_dealer_contract_document(p_contract_id) then raise exception 'forbidden' using errcode = '42501'; end if;
  ...
  if p_document_kind = 'final' then
    ...
    if coalesce(locked_snapshot ->> 'contractLanguage', 'da') <> p_language then raise exception 'final document language must match locked snapshot' using errcode = '22023'; end if;
    if p_language <> 'da' then raise exception 'legal translation not production ready' using errcode = '22023'; end if;
  end if;
  select coalesce(max(document_version), 0) + 1 into next_version from public.dealer_contract_document_versions where contract_id = p_contract_id;
  insert into public.dealer_contract_document_versions (contract_id, document_version, template_version, language, document_kind, snapshot, storage_path, file_name)
  values (p_contract_id, next_version, p_template_version, p_language, p_document_kind, case when p_document_kind = 'final' then locked_snapshot else p_snapshot end, '', p_file_name)
  ...
  perform public.audit_dealer_contract_event(p_contract_id, 'contract_pdf_prepared', jsonb_build_object('document_id', result.id, 'document_version', result.document_version, 'document_kind', p_document_kind, 'template_version', p_template_version, 'language', p_language));
  return result;
end;
$$;
...
set search_path = public, storage
as $$
declare result public.dealer_contract_document_versions;
begin
  ...
  if p_file_size is null or p_file_size < 1 or p_page_count is null or p_page_count < 1 then raise exception 'file metadata missing' using errcode = '22023'; end if;
  if not exists (select 1 from storage.objects o where o.bucket_id = result.storage_bucket and o.name = result.storage_path) then raise exception 'stored PDF missing' using errcode = 'P0002'; end if;
  update public.dealer_contract_document_versions
  set status = case when result.document_kind = 'final' then 'final' else 'draft' end, sha256 = p_sha256, file_size = p_file_size, page_count = p_page_count, finalized_at = now()
  where id = p_document_id returning * into result;
  perform public.audit_dealer_contract_event(result.contract_id, 'contract_pdf_generated', jsonb_build_object('document_id', result.id, 'document_version', result.document_version, 'sha256', result.sha256, 'page_count', result.page_count, 'document_kind', result.document_kind));
  return result;
end;
$$;
