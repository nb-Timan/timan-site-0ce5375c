...
begin
  ...
  set status = 'approved', reviewed_at = now(), reviewed_by_user_id = auth.uid(),
      reviewed_by_name = actor ->> 'display_name', reviewed_by_email = actor ->> 'email', updated_at = now()
  ...
  where contract_id = uv.contract_id and id <> uv.id and status in ('draft', 'changes_requested', 'submitted');

  update public.dealer_contracts
  set contract_status = 'approved', status = 'Signed', approved_upload_version_id = uv.id, approved_at = now(),
      approved_by_user_id = auth.uid(), approved_by_name = actor ->> 'display_name',
      approved_by_email = actor ->> 'email', signed_at = now(), archived_at = coalesce(archived_at, now()), updated_at = now()
  where id = uv.contract_id and contract_status = 'submitted_for_approval'
  ...
  standard_discount := case when coalesce(form_terms ->> 'standardMachineDiscountPct', '') ~ '^[0-9]+(\.[0-9]+)?$'
    then least(100::numeric, greatest(0::numeric, (form_terms ->> 'standardMachineDiscountPct')::numeric)) else null end;
  importer_discount := case when coalesce(form_terms ->> 'importerDiscountPct', '') ~ '^[0-9]+(\.[0-9]+)?$'
    then least(100::numeric, greatest(0::numeric, (form_terms ->> 'importerDiscountPct')::numeric)) else null end;
  spare_parts_discount := case when coalesce(form_terms ->> 'sparePartsDiscountPct', '') ~ '^[0-9]+(\.[0-9]+)?$'
    then least(100::numeric, greatest(0::numeric, (form_terms ->> 'sparePartsDiscountPct')::numeric)) else null end;
  ...
  set standard_machine_discount_pct = coalesce(standard_discount, standard_machine_discount_pct),
      ...
      payment_terms = canonical_payment_terms, updated_at = now()
