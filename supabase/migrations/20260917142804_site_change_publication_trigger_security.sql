-- Keep public feature projection writes inside the existing trigger. Internal
-- changelog rows remain protected by their RLS policies (Backend/news_manage).
...
begin
  ...
  if new.status = 'published' then
    ...
    insert into public.site_change_public_entries (
      id, published_at, implemented_at, title, description, localized_content,
      module, change_type, affected_roles, is_important, source_ref, updated_at
    )
    values (
      new.id, coalesce(new.published_at, now()), new.implemented_at, public_title,
      public_description, public_localized_content, new.module, new.change_type,
      coalesce(new.affected_roles, array['all']::text[]), new.is_important,
      new.source_ref, now()
    )
