-- Dealer machine lists use the same canonical registry RPC. This signature
-- adds server-side demo filtering and fields needed for dealer-table search/sort.
...
with canonical as (
  select distinct on (upper(regexp_replace(wr.machine_serial_number,'[^A-Za-z0-9]+','','g')))
    ...
    wr.machine_model, coalesce(dealer.company_name,wr.dealer_name_snapshot) dealer_name, wr.dealer_account_number dealer_number,
    wr.customer_name, null::text order_number, coalesce(wr.is_demo,false) is_demo, wr.delivery_date,
    wr.legacy_operating_hours operating_hours, coalesce(wr.legacy_last_activity_at,wr.registration_date,wr.delivery_date::timestamptz,wr.created_at) latest_activity_at,
    case when wr.source='legacy_machine_import' then wr.legacy_warranty_reference when wr.sharepoint_form_id is not null then 'SP-'||wr.sharepoint_form_id::text else 'SP-'||coalesce(wr.sharepoint_item_id,left(wr.id::text,8)) end warranty_id,
  ...
  select * from assessed where
    (nullif(trim(p_query),'') is null or serial ilike '%'||trim(p_query)||'%' or warranty_id ilike '%'||trim(p_query)||'%' or machine_model ilike '%'||trim(p_query)||'%' or coalesce(order_number,'') ilike '%'||trim(p_query)||'%')
  ...
  select *,count(*) over()::integer filtered_total,row_number() over(order by
    ...
    case when p_sort='order' and p_direction='asc' then order_number end asc nulls last,case when p_sort='order' and p_direction='desc' then order_number end desc nulls last,
