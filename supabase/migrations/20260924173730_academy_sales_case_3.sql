-- Add Sales Case 3 to future and active assigned curricula. Completed cycles
-- remain frozen through academy_cycles.completed_curriculum.
...
as $$
  select case when coalesce('academy' = any(p_modules), false) then
    ...
    || case when coalesce(nullif(p_permissions->'academy_track_sales', 'null'::jsonb), 'true'::jsonb) = 'true'::jsonb then
      array['sales.case_1_rc1000', 'sales.case_2_video_3330', 'sales.case_3_rc1000_delivery', 'crm.part_1', 'crm.part_2']::text[]
      else '{}'::text[] end
