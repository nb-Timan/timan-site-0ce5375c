-- A saved Configurator T-number is already a commercial opportunity. The
-- lead starts as an ordinary open lead and only becomes "Offer sent" when
-- quote_sent_at is written after a verified mail delivery.
...
begin
  ...
  if not found
     or v_configuration.quote_number is null
  ...
  v_is_sent := v_configuration.quote_sent_at is not null;
  v_first_contact_date := coalesce(
    v_configuration.quote_sent_at,
    v_configuration.created_case_at,
    v_configuration.created_at,
    now()
  )::date;
  ...
  if v_lead_id is null then
    ...
    insert into public.crm_leads (
      ...
      v_first_contact_date,
      v_configuration.delivery_date,
      null,
      v_machine_types,
      case when v_is_sent then 'Offer sent to the customer' else 'New lead' end,
      nullif(v_contact_information, ''),
      v_country,
      case when v_is_sent then 'Tilbud afgivet via konfiguratoren' else 'Tilbud oprettet i konfiguratoren' end
        || case when v_configuration.quote_number is not null then ' — ' || v_configuration.quote_number else '' end,
      coalesce(v_configuration.total_price, 0),
      coalesce(v_configuration.total_price, 0),
      case when v_is_sent then 'configurator_sent_quote' else 'configurator_saved_quote' end,
      now(),
      case when v_is_sent then 70 else 10 end,
      case when v_is_sent then 'Offer sent' else 'Lead' end,
      'open',
      true,
    ...
    update public.configurations
     ...
     where id = v_configuration.id
       and lead_id is null;
  elsif v_is_sent then
    -- A manually closed lead is authoritative. Sending transitions the same
    -- linked lead; it never creates a second sales opportunity.
...
-- The existing trigger now also observes quote_number, so the first saved
-- T-number creates the lead before any email is sent.
...
create trigger sync_sent_configuration_quote_to_lead
  after insert or update of quote_number, quote_sent_at, lead_id, order_sent_at, submitted_at, document_type, case_status, status
  on public.configurations
  for each row execute function public.sync_sent_configuration_quote_to_lead();

-- One-time, idempotent repair for existing active saved quotes, including
-- T-4007. Rows already linked to a lead are simply returned unchanged.
