-- Backend-only module usage analytics for Portal Analytics.
-- Returns aggregated data only; raw portal_module_usage rows stay server-side.

create or replace function public.get_backend_user_activity_analytics(
  p_user_id uuid default null,
  p_role text default null,
  p_dealer_number text default null,
...
begin
  ...
  with filtered_usage as (
    ...
    where pmu.last_active_at >= v_from
      ...
      and (p_user_id is null or pmu.user_id = p_user_id)
      and (nullif(p_role, '') is null or pmu.portal_role = p_role)
      and (nullif(p_dealer_number, '') is null or pmu.dealer_number = p_dealer_number)
      and (nullif(p_module_key, '') is null or pmu.module_key = p_module_key)
  ),
  user_rollup as (
    select
      fu.user_id,
  ...
  comparison_scope as (
    select *
    from public.portal_module_usage pmu
    where (p_user_id is null or pmu.user_id = p_user_id)
      and (nullif(p_role, '') is null or pmu.portal_role = p_role)
      and (nullif(p_dealer_number, '') is null or pmu.dealer_number = p_dealer_number)
      and (nullif(p_module_key, '') is null or pmu.module_key = p_module_key)
  ),
...
revoke all on function public.get_backend_user_activity_analytics(uuid, text, text, text, integer) from public, anon;
grant execute on function public.get_backend_user_activity_analytics(uuid, text, text, text, integer) to authenticated, service_role;
