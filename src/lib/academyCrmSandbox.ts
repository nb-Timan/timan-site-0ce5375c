...
type State = { leads: AcademyLead[]; demos: CrmDemoLead[]; part1Started: boolean; part2Started: boolean; dialogOpened: boolean; selectedPartnerId: string | null; demoCreated: boolean };
const KEY = 'timan.academy.crm-leads.v1';
const future = () => new Date(Date.now() + 14 * 86400000).toISOString().slice(0, 10);
const past = () => new Date(Date.now() - 86400000).toISOString().slice(0, 10);
const initial = (): State => ({
  demos: [], part1Started: false, part2Started: false, dialogOpened: false, selectedPartnerId: null, demoCreated: false,
  leads: [
...
export const academyCrmSandbox = {
  ...
  listDealers() { return [rowToDealer({ id: ACADEMY_CRM_PARTNER.id, account_number: 'ACADEMY-01', company_name: ACADEMY_CRM_PARTNER.name, customer_type: 'service_partner', country: 'DK', city: 'Aalborg', assigned_seller_id: 'academy-local-sales-user', assigned_seller_email: 'academy.sales@localhost', assigned_seller_initials: 'AS' })]; },
  ...
  updateCrmLead(id: string, patch: CrmLeadPatch): CrmLead {
    ...
    const current = read();
    write({ ...current, leads: current.leads.map((lead) => lead.id === id ? { ...lead, record: { ...lead.record, ...patch } } : lead) });
  ...
  createCrmDemoLead(input: NewCrmDemoLead): CrmDemoLead {
    if (!this.isActive()) throw new Error('Academy CRM writes must never use production persistence.');
    const sourceId = input.source_lead_id;
    if (!sourceId || !this.getCrmLead(sourceId)) throw new Error('Academy source lead was not found.');
    const existing = read().demos.find((demo) => demo.source_lead_id === sourceId);
    if (existing) return existing;
    const demo: CrmDemoLead = { ...input, id: `academy-demo-${sourceId}`, demo_no: 9901, created_at: new Date().toISOString(), source: 'user' };
    this.convertToDemo(sourceId);
    const current = read();
    write({ ...current, demos: [...current.demos, demo] });
    return demo;
  },
  ...
  getProgress() { const state = read(); const overdue = state.leads.find((lead) => lead.id === 'academy-overdue-lead')!; const configurator = state.leads.find((lead) => lead.id === 'academy-configurator-lead')!; const demo = state.leads.find((lead) => lead.id === 'academy-demo-lead')!; const part1Completed = overdue.saved && isFuture(overdue.nextFollowup) && configurator.saved && !configurator.incomplete; const demoPersisted = state.demos.some((row) => row.source_lead_id === demo.id); const part2Completed = part1Completed && demoPersisted && demo.saved && !!demo.activity.trim() && demo.shared && demo.convertedToDemo && state.demoCreated; return { overdueUpdated: overdue.saved && isFuture(overdue.nextFollowup), configuratorCompleted: configurator.saved && !configurator.incomplete, activityUpdated: demo.saved && !!demo.activity.trim(), dialogOpened: state.dialogOpened, shared: demo.shared && !!state.selectedPartnerId, demoConverted: demoPersisted && demo.convertedToDemo && state.demoCreated, part1Completed, part2Completed }; },
