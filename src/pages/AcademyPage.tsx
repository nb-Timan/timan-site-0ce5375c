...
function AcademyRow({ image, title, description, state, action, onClick }: {
  ...
  return (
    <div className="grid grid-cols-[56px_minmax(0,1fr)_auto] items-center gap-3 border-t border-slate-100 px-4 py-3.5 sm:px-5">
      ...
      <div className="min-w-0">
        ...
        <p className="mt-0.5 line-clamp-2 text-xs leading-5 text-slate-500">{description}</p>
      </div>
...
export default function AcademyPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      />
      <main className="mx-auto w-full max-w-[1760px] px-4 py-5 sm:px-6 lg:px-8 xl:px-10">
        <div className="mx-auto w-full max-w-[1600px]">
          <section className="relative overflow-hidden rounded-xl border border-slate-200 bg-white px-6 py-7 shadow-sm sm:px-8">
            <div className="relative z-10 max-w-2xl">
              <h1 className="text-3xl font-bold text-slate-900 sm:text-4xl">Min Academy</h1>
              <p className="mt-2 max-w-xl text-sm leading-6 text-slate-600">Academy træningsmiljø. Du arbejder med træningsdata. Ingen rigtige kunder, mails eller salgsdata påvirkes.</p>
            </div>
            <img src="/messe/machines/rc-1000s-tile.png" alt="" className="pointer-events-none absolute right-3 top-1/2 hidden h-[135%] w-80 -translate-y-1/2 object-contain opacity-90 xl:block" />
          </section>

          {params.get('locked') && (
            <div className="mt-5 flex items-center gap-2 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-3">
            <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="mt-5 text-3xl font-bold text-slate-900">{progress.completedCount} / {progress.total}</div>
              <ProgressBar value={progress.percentage} />
              <p className="mt-3 text-xs font-medium text-slate-500">{progress.percentage}% gennemført</p>
            </section>
            <section className="rounded-xl border border-amber-300 bg-amber-50/70 p-5 shadow-sm">
              <div className="flex items-center gap-2 text-sm font-bold text-slate-900"><Lock className="h-4 w-4 text-amber-700" />Næste oplåsning: Konfigurator</div>
              <p className="mt-4 min-h-10 text-sm leading-5 text-slate-700">{unlocked ? 'Konfigurator er nu låst op.' : 'Gennemfør Sales Case 1 for at få adgang til den rigtige konfigurator.'}</p>
              <ProgressBar value={configurator.total ? configurator.completedCount / configurator.total * 100 : 0} gold />
              <p className="mt-3 text-xs font-bold text-slate-700">{configurator.completedCount} / {configurator.total} gennemført</p>
            </section>
            <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <p className="mt-5 text-lg font-bold text-slate-900">Bronze</p>
              <p className="mt-1 text-sm text-slate-600">Gennemfør grundlæggende Sales-opgaver.</p>
              <ProgressBar value={progress.percentage} />
              <p className="mt-3 text-xs font-bold text-slate-600">{progress.completedCount} / {progress.total}</p>
            </section>
          </div>
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-3">
            <section className="relative overflow-hidden rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="relative z-10">
                ...
                <p className="mt-4 text-sm font-bold text-slate-900">Case 1 - Byg korrekt RC-1000 ordre</p>
                <p className="mt-1 text-xs text-slate-500">{requirements} af 10 krav opfyldt</p>
                <button type="button" onClick={startCase} className="mt-4 inline-flex items-center gap-2 rounded-md bg-[#126a45] px-3.5 py-2 text-xs font-bold text-white shadow-sm hover:bg-[#0f5a3b]">
                  {task.started ? 'Fortsæt' : 'Start'}
                  <ArrowRight className="h-3.5 w-3.5" />
                </button>
              </div>
              <img src="/messe/machines/rc-1000s-tile.png" alt="" className="pointer-events-none absolute -bottom-5 right-0 h-36 w-36 object-contain" />
            </section>
            <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-center gap-2 text-sm font-bold text-slate-900"><Medal className="h-4 w-4 text-[#126a45]" />Badges / aktivitet</div>
              <div className="mt-4 space-y-3 text-xs">
                <div className="flex items-center gap-3"><Medal className="h-7 w-7 text-[#b77939]" /><span><b className="block text-slate-800">Bronze</b><span className="text-slate-500">Grundlæggende portalflows</span></span></div>
                <div className="flex items-center gap-3"><ShieldCheck className="h-7 w-7 text-slate-400" /><span><b className="block text-slate-800">Sølv</b><span className="text-slate-500">Videregående Sales-flows</span></span></div>
                <div className="flex items-center gap-3"><Crown className="h-7 w-7 text-amber-500" /><span><b className="block text-slate-800">Guld</b><span className="text-slate-500">Komplet end-to-end workflow</span></span></div>
              </div>
            </section>
            <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-5 space-y-4">
            ...
            <Module icon={Users} title="CRM" progress="0 / 2 gennemført"><AcademyRow title="CRM Leads - grundforløb" description="Kommer senere." state="locked" /></Module>
            <Module icon={CalendarDays} title="Kalender" progress="0 / 1 gennemført"><AcademyRow title="Kalender - grundforløb" description="Kommer senere." state="locked" /></Module>
          </div>

          <Link className="mt-6 inline-flex items-center gap-2 text-xs font-semibold text-[#126a45] hover:underline" to="/portal">← Tilbage til portalen</Link>
        </div>
      </main>
    </div>
  );
