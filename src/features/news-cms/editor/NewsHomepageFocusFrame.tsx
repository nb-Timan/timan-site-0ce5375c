...
export default function NewsHomepageFocusFrame({ imageUrl, headline, focus, imageTransform, onFocusChange }: Props) {
  const activeFocus = readNewsHomepageFocus(focus) || DEFAULT_NEWS_HOMEPAGE_FOCUS;
  const activeTransform = readNewsImageTransform(imageTransform);
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">Billedplaceringen er låst. Træk kun den stiplede ramme til det udsnit, der bruges på forsiden.</p>
        </div>
        <button
          type="button"
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_190px]">
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-slate-100 p-2">
          <p className="mb-2 text-[11px] font-bold uppercase tracking-wide text-slate-500">Låst billedkomposition</p>
          <div className="relative aspect-video overflow-hidden rounded-lg bg-slate-200">
            <img
              src={imageUrl}
              alt=""
              draggable={false}
              className={`h-full w-full ${activeTransform ? 'object-contain' : 'object-cover'}`}
              style={activeTransform ? {
                transform: `translate(${activeTransform.x}%, ${activeTransform.y}%) scale(${activeTransform.scale})`,
                transformOrigin: 'center',
              } : undefined}
            />
            <NewsHomepageFocusOverlay focus={activeFocus} onFocusChange={onFocusChange} />
          </div>
        </div>
