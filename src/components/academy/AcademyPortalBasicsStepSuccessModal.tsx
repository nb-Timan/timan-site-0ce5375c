  ...
  return <Dialog open={Boolean(success)} onOpenChange={(open) => { if (!open) close(); }}>
    <DialogContent className="max-w-sm text-center">
      <DialogHeader className="items-center text-center">
        ...
        <DialogTitle className="text-lg">Godt gået!</DialogTitle>
        <DialogDescription className="text-sm leading-6 text-slate-600">
          Du har gennemført:<br />
          <strong className="font-semibold text-slate-900">{success?.title}</strong>
        </DialogDescription>
      </DialogHeader>
      <p className="text-sm font-semibold text-emerald-800">{success?.completed} af {success?.total} hurtige gennemført.</p>
