...
import { CheckCircle2, CircleAlert, FileText, Film, Image, Save, Send, Upload, X } from 'lucide-react';
...
export const MARKETING_BADGE_OPTIONS = ['', 'Ny', 'Godt køb', 'Vigtigt', 'Kampagne', 'Egen tekst'] as const;
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        ...
        {item && draft && <div className="space-y-5">
          ...
          <div className="flex flex-wrap gap-4 rounded-md border border-slate-200 px-3 py-2">
            ...
            <AssetState label="Specifikationer" published={Boolean(item.defaults.specification_url || item.defaults.specs.length)} draft={Boolean(draft.specification_url || draft.specs.length)} />
          </div>
          ...
          <Field label="Beskrivelse"><Textarea value={draft.description} onChange={(event) => setDraft({ ...draft, description: event.target.value })} /></Field>
          ...
          <Field label="Specifikationslink"><Input value={draft.specification_url} onChange={(event) => setDraft({ ...draft, specification_url: event.target.value })} placeholder="https://..." /></Field>
          <Field label="Badge"><select value={customBadge ? 'Egen tekst' : draft.badge} onChange={(event) => { const selected = event.target.value; setCustomBadge(selected === 'Egen tekst'); setDraft({ ...draft, badge: selected === 'Egen tekst' ? '' : selected }); }} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm">{MARKETING_BADGE_OPTIONS.map((option) => <option key={option || 'none'} value={option}>{option || 'Ingen'}</option>)}</select></Field>
