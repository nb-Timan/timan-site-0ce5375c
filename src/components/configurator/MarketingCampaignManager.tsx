...
import type { CampaignProductRole, CampaignType, ProductCampaign } from '@/lib/configuratorCampaigns';

type Props = { catalog: MarketingConfiguratorCatalogItem[]; language: PortalUiLanguage };
...
export default function MarketingCampaignManager({ catalog, language }: Props) {
  ...
  const refresh = async (selectedId?: string | null) => {
    ...
    if (selectedId) setDraft(result.rows.find(row => row.id === selectedId) ?? null);
  };
  ...
  return <>
    <div className="mx-auto mb-4 flex max-w-6xl justify-end">
      <Button type="button" onClick={() => setOpen(true)}><Megaphone className="mr-2 h-4 w-4" />{T('campaignManager')}</Button>
    </div>
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="max-h-[94vh] overflow-hidden sm:max-w-6xl">
        ...
        <div className="grid min-h-0 gap-5 overflow-hidden lg:grid-cols-[17rem_minmax(0,1fr)]">
          <aside className="min-h-0 space-y-2 overflow-y-auto border-r pr-4">
            <Button className="w-full" variant="outline" onClick={() => { setDraft(emptyMarketingCampaign()); setMessage(null); }}><Plus className="mr-2 h-4 w-4" />{T('campaignNew')}</Button>
          ...
          <section className="min-h-0 overflow-y-auto pr-1">
            {!draft ? <div className="py-16 text-center text-sm text-slate-500">{T('campaignNew')}</div> : <div className="space-y-4">
              {message && <div className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm">{message}</div>}
              ...
              {(['trigger', 'benefit', 'linked'] as CampaignProductRole[]).map(role => {
                ...
                return <div key={role} className="space-y-2"><h3 className="text-sm font-semibold">{roleTitle(role)}</h3>{products.map(product => { const item = catalogByKey.get(product.productKey); return <div key={`${role}-${product.productKey}`} className="flex items-center justify-between rounded-md border px-3 py-2 text-sm"><span><strong>{product.itemNumber}</strong> · {item?.defaults.title || product.productKey}</span><Button size="icon" variant="ghost" onClick={() => removeProduct(product.productKey, role)} aria-label={T('campaignRemoveProduct')}><X className="h-4 w-4" /></Button></div>; })}</div>;
              })}
            </div>}
          </section>
        </div>
