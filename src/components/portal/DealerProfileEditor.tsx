 ...
 *
 * Renders the 6 onboarding sections, a top-level progress indicator and
 * a per-section save button. Backed by:
 *   - dealer_accounts (phase50/52 RLS: dealer-side can read/update own row)
 *   - dealer_contacts (phase52)
 *
 * No pricing / quote / order logic touched. All strings come from
 * dealerProfileI18n so DK/GB/DE/IT/HU work out of the box.
 */
...
export default function DealerProfileEditor({ dealer, language, canEdit, onUpdated }: Props) {
  const t = (k: ProfileI18nKey) => tProfile(language, k);
  ...
  useEffect(() => { setDraft(dealer); }, [dealer]);
  ...
  // -------- shared inputs --------
  const Field = ({ id, label, value, onChange, required, type = "text" }: {
    id: string; label: string; value: string | null; required?: boolean;
    ...
    <div>
      ...
      <Input id={id} type={type} value={value ?? ""} disabled={!canEdit}
        onChange={(e) => onChange(e.target.value)} />
    </div>
  );

  const SectionShell = ({ skey, title, children, onSave }: {
    skey: SectionKey; title: string; children: React.ReactNode; onSave: () => void;
  }) => {
    const status = completion.sections.find((s) => s.key === skey)!;
    const badge = status.complete
      ? <Badge className="bg-emerald-100 text-emerald-800 hover:bg-emerald-100"><CheckCircle2 className="h-3 w-3 mr-1" />{t("statusComplete")}</Badge>
      : status.filled > 0
      ? <Badge className="bg-amber-100 text-amber-800 hover:bg-amber-100">{t("statusPartial")} ({status.filled}/{status.required})</Badge>
        ...
        <CardContent className="space-y-4">
          ...
          {canEdit && (
            <div className="flex justify-end">
              <Button size="sm" onClick={onSave} disabled={savingSection === skey}>
                {savingSection === skey ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Save className="h-4 w-4 mr-2" />}
                {savingSection === skey ? t("saving") : t("save")}
              </Button>
            </div>
          )}
        </CardContent>
    ...
    <div className="space-y-6">
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field id="company_name" label={t("companyName")} value={dealer.company_name} onChange={() => {}} required />
          <Field id="director_name" label={t("directorName")} value={draft.director_name} onChange={(v) => set("director_name", v)} required />
          <Field id="address" label={t("address")} value={draft.address} onChange={(v) => set("address", v)} required />
          <div className="grid grid-cols-2 gap-3">
            <Field id="postal_code" label={t("postalCode")} value={draft.postal_code} onChange={(v) => set("postal_code", v)} required />
            <Field id="city" label={t("city")} value={draft.city} onChange={(v) => set("city", v)} required />
          </div>
          <Field id="country" label={t("country")} value={draft.country} onChange={(v) => set("country", v)} required />
          <Field id="vat_number" label={t("vatNumber")} value={draft.vat_number} onChange={(v) => set("vat_number", v)} required />
          <Field id="phone" label={t("phone")} value={draft.phone} onChange={(v) => set("phone", v)} required />
          <Field id="email" label={t("email")} value={draft.email} onChange={(v) => set("email", v)} type="email" required />
        </div>
      </SectionShell>

      {/* 2) Finance */}
      <SectionShell
        skey="finance" title={t("sec2")}
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field id="finance_contact_name" label={t("financeContactName")} value={draft.finance_contact_name} onChange={(v) => set("finance_contact_name", v)} required />
          <Field id="invoice_email" label={t("invoiceEmail")} value={draft.invoice_email} onChange={(v) => set("invoice_email", v)} type="email" required />
          <Field id="finance_contact_phone" label={t("financePhone")} value={draft.finance_contact_phone} onChange={(v) => set("finance_contact_phone", v)} />
          <Field id="finance_contact_email" label={t("financeEmail")} value={draft.finance_contact_email} onChange={(v) => set("finance_contact_email", v)} type="email" />
        </div>
      </SectionShell>

      {/* 3) Media */}
      <SectionShell
        skey="media" title={t("sec3")}
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field id="website" label={t("website")} value={draft.website} onChange={(v) => set("website", v)} required />
          <Field id="social_facebook"  label={t("facebook")}  value={draft.social_facebook}  onChange={(v) => set("social_facebook", v)} />
          <Field id="social_linkedin"  label={t("linkedin")}  value={draft.social_linkedin}  onChange={(v) => set("social_linkedin", v)} />
          <Field id="social_tiktok"    label={t("tiktok")}    value={draft.social_tiktok}    onChange={(v) => set("social_tiktok", v)} />
          <Field id="social_youtube"   label={t("youtube")}   value={draft.social_youtube}   onChange={(v) => set("social_youtube", v)} />
          <Field id="social_instagram" label={t("instagram")} value={draft.social_instagram} onChange={(v) => set("social_instagram", v)} />
        </div>
      </SectionShell>

      {/* 4) Sales */}
      <SectionShell
        skey="sales" title={t("sec4")}
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Field id="sales_contact_name"  label={t("salesContactName")} value={draft.sales_contact_name}  onChange={(v) => set("sales_contact_name", v)} />
          <Field id="sales_contact_phone" label={t("salesPhone")}       value={draft.sales_contact_phone} onChange={(v) => set("sales_contact_phone", v)} />
          <Field id="sales_contact_email" label={t("salesEmail")}       value={draft.sales_contact_email} onChange={(v) => set("sales_contact_email", v)} type="email" />
        </div>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Field id="workshop_contact_name"  label={t("workshopContactName")} value={draft.workshop_contact_name}  onChange={(v) => set("workshop_contact_name", v)} />
          <Field id="workshop_contact_phone" label={t("workshopPhone")}       value={draft.workshop_contact_phone} onChange={(v) => set("workshop_contact_phone", v)} />
          <Field id="workshop_contact_email" label={t("workshopEmail")}       value={draft.workshop_contact_email} onChange={(v) => set("workshop_contact_email", v)} type="email" />
        </div>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Field id="marketing_contact_name"  label={t("marketingContactName")} value={draft.marketing_contact_name}  onChange={(v) => set("marketing_contact_name", v)} />
          <Field id="marketing_contact_phone" label={t("marketingPhone")}       value={draft.marketing_contact_phone} onChange={(v) => set("marketing_contact_phone", v)} />
          <Field id="marketing_contact_email" label={t("marketingEmail")}       value={draft.marketing_contact_email} onChange={(v) => set("marketing_contact_email", v)} type="email" />
        </div>
      </SectionShell>
    </div>
  );
